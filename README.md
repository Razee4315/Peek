...
GitHub Pages is static hosting, so the host browser owns the match. PeerJS handles signaling. A Cloudflare Worker issues temporary TURN credentials before each connection. Only its public `VITE_RELAY_URL` is embedded in the frontend; the provider API token stays in Worker secrets. Online play needs that Worker deployed and configured. The old bundled PeerJS relays were discontinued. See [relay setup](docs/07-relay-setup.md). The guest receives only a seat-scoped view, never the opponent's secret before the result. Submitted guesses and their hints are shared with both players; unsubmitted drafts remain private. Public relay availability and restrictive firewalls can still affect connections. Credential fetching is bounded to eight seconds and connection negotiation times out after a further 25 seconds with an actionable error and can be cancelled.
