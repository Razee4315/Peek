...
describe('room transport lifecycle', () => {
  ...
  it('keeps a deadline until welcome, not just channel open', () => {
    const h = guestHandlers()
    new GuestConnection().join('ABCDE', h)
    const peer = fake.peers[0]
  ...
  it('clears the deadline on welcome and reports a later disconnect once', () => {
    const h = guestHandlers()
    new GuestConnection().join('ABCDE', h)
    const peer = fake.peers[0]
    peer.emit('open')
  ...
  it('cancels old attempts without allowing stale errors into a new join', () => {
    const guest = new GuestConnection(), old = guestHandlers(), next = guestHandlers()
    guest.join('ABCDE', old)
    const peer = fake.peers[0]
    guest.join('FGHJK', next)
  ...
  it('reserves a pending seat and ignores commands before hello', () => {
    const h = hostHandlers()
    new HostRoom().start('ABCDE', h)
  ...
  it('releases an unresponsive guest reservation so a new guest can join', () => {
    const h = hostHandlers()
    new HostRoom().start('ABCDE', h)
    const peer = fake.peers[0], first = new fake.Connection(), second = new fake.Connection()
  ...
  it('bounds room creation and suppresses close callbacks after intentional shutdown', () => {
